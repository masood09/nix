...
in {
  ...
  config = lib.mkMerge [
    ...
    # GPU driver stack — decoupled from homelab.desktop.enable so headless
    # servers (e.g. Jellyfin hardware transcoding) can pull in VA-API/VDPAU
    # support without any of the rest of the desktop config.
    (lib.mkIf gfxCfg.enable {
      hardware = {
        graphics = {
          enable = true;
      ...
      # the fixed /etc/OpenCL/vendors path instead, which nothing creates on its own.
      # Without this, any OpenCL consumer (e.g. ffmpeg's tonemap_opencl filter) fails
      # with "Failed to get number of OpenCL platforms: -1001" despite the runtime
      # package being installed.
      environment.etc = lib.mkIf isIntel {
        "OpenCL/vendors".source = "${pkgs.intel-compute-runtime}/etc/OpenCL/vendors";
      };
    })
  ];
}

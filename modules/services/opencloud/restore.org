...
Dataset names are derived from their mountpoints, so this is correct
regardless of which pool each lives on (on heartbeat these span both fpool
and dpool):

#+begin_src bash
for mp in /var/lib/opencloud/etc /var/lib/opencloud/idm \
          /var/lib/opencloud/storage/metadata \
          /var/lib/opencloud/storage/ocm \
          /var/lib/opencloud/storage/users; do

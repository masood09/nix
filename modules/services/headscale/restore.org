...
whichever host runs headscale (meshcontrol backs =/var/lib/headscale= with an
rpool dataset, trialunit with a dpool one):

#+begin_src bash
ds=$(zfs list -H -o name,mountpoint | awk '{p=$2;sub(/\/+$/,"",p);if(p=="/var/lib/headscale")print $1}')

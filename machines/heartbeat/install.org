...
=heartbeat= physical server using:

- NixOS Live ISO (booted via IPMI virtual media)
- ZFS root with Disko
- Encrypted ZFS pools (rpool, dpool, fpool)
...
- Boot the NixOS Live ISO via IPMI virtual media
...
- IPMI access to heartbeat
- NixOS Live ISO (built via =just build-iso=)
- GitHub repo with NixOS flake (=github:masood09/nix=)

* 1. Boot NixOS Live ISO via IPMI

1. Log in to the IPMI web interface
2. Mount the NixOS ISO via virtual media
3. Reboot the server and boot from the virtual CD/DVD
...
#+begin_example
-r-------- 1 root root 64 dpool_data.key
-r-------- 1 root root 64 fpool_fast.key
#+end_example
